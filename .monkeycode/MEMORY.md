...
[赛事统计更新流程]
- Date: 2026-08-27
- Context: 用户明确赛事栏目维护流程；Agent 更新数据时发现测试耦合
- Category: 工作流协作
- Instructions:
  - 更新赛事统计时依次执行：调研官方来源、收集可核验信息、整理截止日期与状态、将赛事置入当前可报名或历史赛事、修改数据并验证后提交推送。
  - 截止日已过或明确标记结束的赛事归入历史赛事；仅在主办方或赛事平台官网可核验后列为当前可报名赛事。
  - 赛事数据位于 `lib/competitions.ts`：`competitionDataVerifiedAt` 是核验日期常量，所有记录的 `verifiedAt` 引用该常量。
  - 修改核验日期时必须同步两处测试断言：`lib/competitions.test.ts` 的日期断言、`app/stats/page.test.tsx` 的活跃赛事断言（须引用新核验日期下仍在报名期的赛事，原引用赛事可能已归档）。
  - 已过截止日的赛事由 `resolveCompetitionStatus` 在渲染时自动归档（赛事截止→已结束，报名截止→保留为报名截止状态）；状态变化通过在 `changes` 数组追加带日期的复核记录来显式留痕。
