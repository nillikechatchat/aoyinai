...
`,
  ...
  {
    slug: "function-calling-guide", category: "tutorials", tags: "Function Calling,工具调用,Agent", cover: "/images/cover-tutorials.png", readMinutes: 11, daysAgo: 1,
    title: "工具调用实战：让大模型从「会聊天」到「会办事」",
    excerpt: "模型再聪明，也够不着你的数据库。工具调用（Function Calling）是让模型伸出手的第一课——本文拆解它在工程上真正的难点。",
    content: `## 一、只会说话的模型，办不了事

大模型的知识停在训练那一刻。它不知道你的库存还剩几件，也读不到今天的数据库。工具调用要解决的，正是把外部能力以结构化接口「声明」给模型，让它在需要时主动开口。

《荀子》有言：「君子生非异也，善假于物也。」模型本身的智力或许固定，它能借用的工具却可以无限扩展。

## 二、三段式：声明、抉择、回填

一次完整的工具调用，是「声明契约 → 模型选择 → 结果回填」的循环。声明靠 JSON Schema 描述能力，抉择是模型给出的调用意图（它只提议，不执行），回填则是你在本地执行真实函数，再把精简后的结果还给模型。

## 三、工程上的四道坎

1. **描述即提示**：Schema 里的说明写清「何时用」「何时别用」，比换更强的模型更有效；
2. **参数必校验**：模型输出一律过校验层，失败则带错误重试，绝不直接执行；
3. **结果要收敛**：工具返回值先裁剪摘要，必要时只回关键字段；
4. **循环要设限**：给「调用—回填」设最大轮数，防止陷入死循环烧钱。

其中参数校验最易被忽略——模型把「一百元」填进数字字段是常事。
